...
COMMAND = f'room/{config.ROOM_NAME}/command/accept'
...
def start(core:VACore):
    manifest = { # возвращаем настройки плагина - словарь
        "name": "Accept an incoming call", # имя
        "version": "1.0", # версия
        "require_online": False, # требует ли онлайн?

        "commands": { # набор скиллов. Фразы скилла разделены | . Если найдены - вызывается функция
            "принять|прими|принимай|примай": command2mqtt
        }
    }
    return manifest
...
def command2mqtt(core:VACore, phrase: str): # в phrase находится остаток фразы после названия скилла,
    ...
    time.sleep(1)
    mqtt_client.publish(COMMAND, "")
    time.sleep(1)
    print(f'Publish "{COMMAND}" successfully.')
